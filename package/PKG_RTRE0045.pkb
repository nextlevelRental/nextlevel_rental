CREATE OR REPLACE PACKAGE BODY NXRADMIN.Pkg_Rtre0045 AS
/*******************************************************************************
   NAME      Pkg_Rtre0045
...
*******************************************************************************/
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    SELECT  A.CHECK_DAY,                 /*체크일자            */
            A.CHECK_SET,                 /*회차                */
            A.CSEQ,                      /*순번                */
            SUBSTR(A.ORD_NO, 0, 12) AS ORD_NO,                    /*계약번호            */
            A.CUST_NM,                   /*고객명              */
            A.RECP_PAY,                  /*결제방법            */
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    Pkg_Utility.p_ErrorFileWrite('Pkg_Rtre0045.p_IUDRtre0045(2)', '체크일자', v_Check_Day);
    Pkg_Utility.p_ErrorFileWrite('Pkg_Rtre0045.p_IUDRtre0045(2)', '회차', v_Check_Set);
    Pkg_Utility.p_ErrorFileWrite('Pkg_Rtre0045.p_IUDRtre0045(2)', '순번', v_Cseq);
    Pkg_Utility.p_ErrorFileWrite('Pkg_Rtre0045.p_IUDRtre0045(2)', '등록자 ID', v_Reg_Id);
  ...
  *****************************************************************************/
  PROCEDURE p_IUDRtre0045 (
    ...
    e_Error EXCEPTION;
    v_Recp_Amt  RTRE0045.RECP_AMT%TYPE DEFAULT NULL;       /*수납처리금액          */
    v_Diff_Yn   RTRE0045.DIFF_YN%TYPE DEFAULT NULL;        /*차이여부              */
    v_Cnc_Amt   RTRE0045.CNC_AMT%TYPE := 0;                /*취소금액              */
    v_New_Cncp_Amt   RTRE0045.CNCP_AMT%TYPE := 0;          /*취소가능금액              */
...
--        END IF;
    ...
    v_New_Cncp_Amt := v_Cncp_Amt;
    
    --거래구분
    IF v_App_Desc = '환불' THEN
        v_Cnc_Amt := v_Cncp_Amt;
        v_New_Cncp_Amt := 0;
    END IF;
    
    --거래상태
    IF v_St_Desc = '실패' THEN
        v_New_Cncp_Amt := 0;
    END IF;
    ...
    IF v_Comm_Dvsn = 'I' THEN
        ...
        IF v_Dup_Gb = 'M' THEN
            ...
            IF 0 < f_sRtre0045TnoDupCheck(v_Check_Day, v_Check_Set, v_Tno) THEN

                IF 0 < f_sRtre0045TnoValueCheck(v_Check_Day, v_Check_Set, v_Tno, v_App_Amt, v_New_Cncp_Amt) THEN
        ...
        IF (v_Dup_Skip = FALSE) THEN
            ...
            -- 순번획득
            v_Cseq := f_sRtre0045Cseq(v_Check_Day, v_Check_Set);
            
            IF 0 != f_InsertRtre0045(v_Check_Day, v_Check_Set, v_Cseq, v_Ord_No,
                                     ...
                                     v_New_Cncp_Amt, v_Recp_Amt, v_Diff_Yn, v_Reg_Id,
                                     v_ErrorText) THEN
        ...
        ELSE
            -- 중복 Skip Log 생성
            Pkg_Utility.p_ErrorFileWrite('Pkg_Rtre0045.p_IUDRtre0045(Dup Skip)', v_Comm_Dvsn || '-' || v_Dup_Gb || '-'
                                         ...
                                         || v_New_Cncp_Amt || '-' || v_Reg_Id);
        END IF;


    ELSE
        
        IF 0 = f_sRtre0045Count(v_Check_Day, v_Check_Set, v_Cseq) THEN
        ...
        IF v_Comm_Dvsn = 'U' THEN
            ...
            IF 0 != f_UpdateRtre0045(v_Check_Day, v_Check_Set, v_Cseq, v_Ord_No,
                                     nvl(v_Cust_Nm,' '), v_Recp_Pay, v_Bank_Nm, v_Cardcom_Nm,
                                     v_App_Amt, v_St_Desc, TO_DATE(v_App_Day,'YYYY.MM.DD HH24:MI:SS'), TO_DATE(v_Cnc_Day,'YYYY.MM.DD HH24:MI:SS'),
                                     TO_DATE(v_Pcnc_Day,'YYYY.MM.DD'), v_Tno, v_App_Desc, v_Cnc_Amt,
                                     v_New_Cncp_Amt, v_Recp_Amt, v_Diff_Yn, v_Reg_Id,
        ...
        ELSIF v_Comm_Dvsn = 'D' THEN
            ...
            IF 0 != f_DeleteRtre0045(v_Check_Day, v_Check_Set, v_Cseq, v_Reg_Id,
                                     v_ErrorText) THEN
    ...
    EXCEPTION
      WHEN e_Error THEN
        ...
        Pkg_Utility.p_ErrorFileWrite('Pkg_Rtre0045.p_IUDRtre0045(1)', v_ErrorText, v_Return_Message);
      ...
      WHEN OTHERS THEN
        ...
        v_ErrorText := SUBSTR(SQLERRM, 1, 200);
        Pkg_Utility.p_ErrorFileWrite('Pkg_Rtre0045.p_IUDRtre0045(2)', v_ErrorText, v_Return_Message);
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    -- 처리구분 : 'N' : RTRE0045.DIFF_YN 값이 NULL인 내역만 처리대상임.
    -- 처리구분 : 'O' : RTRE0045.DIFF_YN 값에 관계없이 체크일자, 체크회차에 대항하는 전체 내역이 처리 대상임
    -- TNO(거래번호)를 기준으로 RTRE0041 내역 SUM한 금액과 취소가능금액(CNCP_AMT) 와 비교하여 차이건을 확인한다.

            
    BEGIN
        
        FOR CUR IN (SELECT A.CUST_NO, A.ORD_NO, SUM(A.RECP_AMT) AS RECP_AMT, 'P6' AS RECP_PAY, A.RECV_DAY, B.TNO 
                    FROM RTRE0030 A, RTRE0210 B 
                    ...
                    AND B.RACT_DAY BETWEEN SUBSTR(v_Check_Day, 1, 6) || '01' AND LAST_DAY(TO_DATE(SUBSTR(v_Check_Day, 1, 6) || '01', 'YYYYMMDD'))
                    AND A.RECP_PAY = 'P6'
                    AND A.CNC_STAT = 'N'
            ...
            ELSE
                ...
                SELECT v_Check_Day
                     , v_Check_Set
                     , (SELECT MAX(CSEQ) + 1 FROM RTRE0045 WHERE CHECK_DAY = v_Check_Day AND CHECK_SET = v_Check_Set)
                     , CUR.ORD_NO
    ...
    EXCEPTION
      WHEN e_Error THEN
        ...
        v_Return_Message := v_Return_Message;
        v_ErrorText := SUBSTR(SQLERRM, 1, 200)||':'||TRIM(v_ErrorText);
        Pkg_Utility.p_ErrorFileWrite('Pkg_Rtre0045.p_UpdateRtre0045Check(1)', v_ErrorText, v_Return_Message);
      ...
      WHEN OTHERS THEN
        ...
        v_Return_Message := NVL( TRIM(v_Return_Message), '시스템관리자에게 문의바랍니다!.');
        v_ErrorText := SUBSTR(SQLERRM, 1, 200);
        Pkg_Utility.p_ErrorFileWrite('Pkg_Rtre0045.p_UpdateRtre0045Check(2)', v_ErrorText, v_Return_Message);

  END p_UpdateRtre0045Check;
