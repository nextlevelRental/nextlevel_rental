CREATE OR REPLACE PACKAGE BODY NXRADMIN.Pkg_Rtcm9002 AS
/*******************************************************************************
   NAME      Pkg_Rtcm9002
   PURPOSE   거래테스트
...
--    AND     (A.SCORE IS NULL OR A.SCORE = v_Score)
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    IF v_Comm_Dvsn = 'I' THEN
        IF 0 != f_sRtcm9002Count(v_Answer_No, v_Answer_Seq) THEN
            v_Return_Message := '해당 답변순번('||v_Answer_Seq||')은 이미 등록된 정보이므로 신규등록 처리가 불가 합니다!';
            RAISE e_Error;
        END IF;
        
        IF 0 != f_InsertRtcm9002(v_Answer_No, v_Answer_Seq, v_Use_Yn, v_Answer, 
    ...
    ELSE
        ...
        IF v_Comm_Dvsn = 'U' THEN        
            IF 0 != f_UpdateRtcm9002(v_Answer_No, v_Answer_Seq, v_Use_Yn, v_Answer, 
                                     v_Score, v_Reg_Id, v_ErrorText) THEN
        ...
        ELSIF v_Comm_Dvsn = 'D' THEN
            ...
            IF 0 != f_DeleteRtcm9002(v_Answer_No, v_Answer_Seq, v_Reg_Id, v_ErrorText) THEN
    ...
    EXCEPTION
      WHEN e_Error THEN
        ...
        Pkg_Utility.p_ErrorFileWrite('Pkg_Rtcm9002.p_IUDRtcm9002(1)', v_ErrorText, v_Return_Message);
      ...
      WHEN OTHERS THEN
        ...
        v_ErrorText := SUBSTR(SQLERRM, 1, 200);
        Pkg_Utility.p_ErrorFileWrite('Pkg_Rtcm9002.p_IUDRtcm9002(2)', v_ErrorText, v_Return_Message);
  ...
  *****************************************************************************/
  FUNCTION f_sRtcm9002_JS(
    ...
    v_Score               ZTCS0113.SCORE%TYPE;          /*점수                */
  BEGIN
