...
import egovframework.ntrms.kr.co.brn.re.service.RTRESETTLEVanHisCheckService;
...
@Controller
public class RTRESETTLEVanHisCheckController {
	
	protected Log logger = LogFactory.getLog(this.getClass());	

	@Resource(name="rTRESETTLEVanHisCheckService")
	RTRESETTLEVanHisCheckService rTRESETTLEVanHisCheckService;
	...
	@RequestMapping("/rtms/re/checkSetRtre0046.do") 
	public ModelAndView selectCheckSet(NexacroMapDTO xpDto, Model model) throws Exception {
		ModelAndView mav = new ModelAndView("nexacroMapView");
		try{
			Map <String, Object> inVar 			= xpDto.getInVariableMap();
			Map <String, DataSetMap> outDataset = xpDto.getOutDataSetMap();
			Map result = rTRESETTLEVanHisCheckService.selectCheckSet(inVar, outDataset);
	...
	@RequestMapping("/rtms/re/initRTRESETTLEVanHisCheck.do") 
	public ModelAndView initRTRESETTLEVanHisCheck(NexacroMapDTO xpDto, Model model) throws Exception {
		ModelAndView mav = new ModelAndView("nexacroMapView");
		try{
			Map <String, Object> inVar 			= xpDto.getInVariableMap();
			Map <String, DataSetMap> outDataset = xpDto.getOutDataSetMap();
	...
	public void settleVanUploadExcelFile(NexacroMapDTO xpDto, HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		...
		Map result = null;
		Map resultUpdateRtre0046Check = null;
		...
		try{
			...
			for( int i = 0 ; i < tmpNameList.size() ; i++){
				...
				HSSFWorkbook workbook 	= new HSSFWorkbook(fs);
		       	...
		       	for( int r = 1 ; r < rows ; r++ ){										 //첫번째 행은 패스 -> r=1 부터시작
		       		...
		       		if( (row != null) ){
		       			for( short c = 0; c < 10; c++ ){								 //계좌승인 셀 갯수 -> 33개
		       				XSSFCell cell = row.getCell(c);
		       				//HSSFCell cell = row.getCell(c);
		       				if(cell != null){
		       					if( c == 0 ){	paramMap.put("appDay",format(cell));	}//승인일자
		       					if( c == 1 ){	paramMap.put("appDay", paramMap.get("appDay") + format(cell));	}//승인시간
		       					if( c == 2 ){	paramMap.put("bankNm",format(cell));	}//은행명
		       					if( c == 4 ){	paramMap.put("tno",format(cell));		}//거래번호
		       					if( c == 5 ){	paramMap.put("appAmt",format(cell));	}//거래금액
		       					if( c == 5 ){	paramMap.put("cncpAmt",format(cell));	}//취소가능금액
		       					if( c == 6 ){	paramMap.put("custNm",format(cell));	}//주문자
		       					if( c == 9 ){	paramMap.put("appDesc",format(cell));	}//거래구분
			...
			result = rTRESETTLEVanHisCheckService.settleVanUploadFile(inDataset, inVar);
			...
			if( successCode == 0 ){
				...
				resultUpdateRtre0046Check = rTRESETTLEVanHisCheckService.resultUpdateRtre0046Check(inDataset, inVarUpdate);
				
				resVarList.add(NexacroConstant.ERROR_CODE, resultUpdateRtre0046Check.get("successCode"));
				resVarList.add(NexacroConstant.ERROR_MSG, resultUpdateRtre0046Check.get("returnMessage"));
			}else{
	...
	@RequestMapping(value = "ntrms/re/settleVanUploadFile.do")
	public void settleVanUploadTxtFile(NexacroMapDTO xpDto, HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		DataSetMap tranInfo = xpDto.getTranInfoMap();
		Map <String, Object> inVar = xpDto.getInVariableMap();
		Map <String, Object> inVarUpdate = xpDto.getInVariableMap();
		Map <String, DataSetMap> inDataset = xpDto.getInDataSetMap();
		...
		try{
			...
			for( int i = 0 ; i < tmpNameList.size() ; i++){
				...
				BufferedReader in= new BufferedReader(new FileReader(contextRealPath + strUploadPath + "/" + tmpNameList.get(i))); 
		        String str; 
		        String[] strArr;
		        while ((str =in.readLine()) != null) { 
		        	...
		        	paramMap.put("appDay",strArr[0] + strArr[1]);	//승인일자
   					paramMap.put("bankNm",strArr[2]);	//은행명
   					paramMap.put("tno",strArr[4]);		//거래번호
   					paramMap.put("appAmt",strArr[5]);	//거래금액
   					paramMap.put("cncpAmt",strArr[5]);	//취소가능금액
   					paramMap.put("custNm",strArr[6]);	//주문자
   					paramMap.put("appDesc",strArr[9]);	//거래구분
	...
	@RequestMapping("rtms/re/saveVerRtre0046.do") 
	public ModelAndView saveVerRtre0046(NexacroMapDTO xpDto, Model model, HttpServletRequest req) throws Exception {
		...
		try{
			...
			inVar.put("regId", loginInfo.get("userId"));
			Map result =  rTRESETTLEVanHisCheckService.saveVerProcess(inDataset, inVar);
	...
	@RequestMapping("/rtms/re/selectSRtre0046.do") 
	public ModelAndView selectSRtre0046(NexacroMapDTO xpDto, Model model) throws Exception {
		ModelAndView mav = new ModelAndView("nexacroMapView");
		try{
			Map <String, Object> inVar 			= xpDto.getInVariableMap();
			Map <String, DataSetMap> outDataset = xpDto.getOutDataSetMap();
			Map result = rTRESETTLEVanHisCheckService.selectSRtre0046(inVar, outDataset);
			List selectSRtre0046 = (List)result.get("selectSRtre0046");
			DataSetMap selectSRtre0046Map = new DataSetMap();
			selectSRtre0046Map.setRowMaps(selectSRtre0046);
			outDataset.put("selectSRtre0046Map", selectSRtre0046Map);
