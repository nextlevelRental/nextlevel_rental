    ...
    return function()
    {
        ...
        this.on_create = function()
        {
            ...
            if (Form == this.constructor) {
                this.set_name("RTRESETTLEDirectHisCheck_tab1");
            ...
            obj = new FileUpload("settleDirectUploadFile", "absolute", "160", "160", "491", "50", null, null, this);
            obj.set_taborder("22");
            ...
            obj = new Button("bt_settleDirectAdd", "absolute", "671", "160", "58", "22", null, null, this);
            obj.set_taborder("23");
            obj.set_text("파일추가");
            obj.set_cssclass("btn_WF_module");
            this.addChild(obj.name, obj);

            obj = new Button("bt_settleDirectDelete", "absolute", "671", "184", "58", "22", null, null, this);
            obj.set_taborder("24");
            obj.set_text("파일삭제");
            obj.set_cssclass("btn_WF_module");
            this.addChild(obj.name, obj);
        ...
        // User Script
        this.addIncludeScript("RTRESETTLEDirectHisCheck_tab1.xfdl", "lib::comLib.xjs");
        this.registerScript("RTRESETTLEDirectHisCheck_tab1.xfdl", function() {
        ...
        this.fn_callBack = function (strSvcId,nErrorCode,strErrorMsg){
        	...
        	if( strSvcId == "saveVerRtre0045" ){
        		alert(strErrorMsg);
        	}
        }
        /*|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||*/	
        this.RTRESETTLEDirectHisCheck_tab1_onload = function(obj,e){
        	Ex.core.init(obj); 	
        	this.fn_init();
        }
        this.fn_init = function(){
        	this.settleDirectUploadFile.setEventHandler("onsuccess", this.settleDirectUploadFile_onsuccess, this);
        	this.settleDirectUploadFile.setEventHandler("onerror", this.settleDirectUploadFile_onerror, this);
        	this.fn_getToday();
        	this.fn_initCombo();
        ...
        this.div_search_bt_upload_onclick = function(obj,e){
        	this.FN_settleDirectUploadFile();
        }
        /*|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||*/
        this.div_search_bt_verProcess_onclick = function(obj,e){
        	...
        	if( checkDay == "" ){
        		...
        		var sSvcID        	= "saveVerRtre0045";                    
        		var sController   	= "rtms/re/saveVerRtre0045.do";
        		var sInDatasets   	= "";
        		var sOutDatasets  	= "";
        		var sArgs 			= "";	
        ...
        /*|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||*/
        this.FN_settleDirectUploadFile = function(obj,e){
        	var checkDay = nvl(this.div_search.cal_checkDay.value);
        	var checkSet = nvl(this.div_search.cb_checkSet.value);
        	var dupGb	 = nvl(this.div_search.ra_dupGb.value);
        	var recpPay	 = "P6";
        	var fileCnt1 = this.settleDirectUploadFile.getItemCount(true);
        	...
        	if( checkDay == "" ){
        		...
        		if( confirm( "파일을 저장하시겠습니까?") ){
        			...
        			var sController = "/ntrms/re/settleDirectUploadFile.do?checkDay="+checkDay+"&checkSet="+checkSet+"&dupGb="+dupGb+"&recpPay="+recpPay;
        			this.settleDirectUploadFile.set_uploadurl(sController);
        			var bSucc = this.settleDirectUploadFile.upload(sController);
        			Ex.core.set_wait(true);
        		}
        	}
        }
        this.settleDirectUploadFile_onerror = function(obj,e){
        ...
        this.settleDirectUploadFile_onsuccess = function(obj,e){
        	application.set_httptimeout(60);
        	Ex.core.set_wait(false);
        	alert(e.errormsg);
        	this.reload();
        ...
        this.bt_settleDirectAdd_onclick = function(obj,e){
        	var fileCnt	 = this.settleDirectUploadFile.getItemCount(false);
        	if( fileCnt == 1){
        		this.settleDirectUploadFile.appendItem();
        	}
        }

        this.bt_settleDirectDelete_onclick = function(obj,e){
        	var fileCnt	 = this.settleDirectUploadFile.getItemCount(false);
        	if( fileCnt == 2){
        		this.settleDirectUploadFile.deleteItem(1); 
        	}
        }
        /*|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||*/
        ...
        {
            this.addEventHandler("onload", this.RTRESETTLEDirectHisCheck_tab1_onload, this);
            ...
            this.settleDirectUploadFile.addEventHandler("onerror", this.cardApproveUploadFile_onerror, this);
            this.settleDirectUploadFile.addEventHandler("onsuccess", this.cardApproveUploadFile_onsuccess, this);
            this.bt_settleDirectAdd.addEventHandler("onclick", this.bt_settleDirectAdd_onclick, this);
            this.bt_settleDirectDelete.addEventHandler("onclick", this.bt_settleDirectDelete_onclick, this);

        };

        this.loadIncludeScript("RTRESETTLEDirectHisCheck_tab1.xfdl");
