...
<%
    ...
    /* =   페이지 설명                                                              = */
    /* = -------------------------------------------------------------------------- = */
    /* =   이 페이지는 KCP에서 샘플로 제공한 order.jsp 를 기반으로 작성됨           = */
    /* = -------------------------------------------------------------------------- = */
    /* =   Revision History                                                         = */
    /* =                                                                            = */
    /* =   Ver		owner		desc.                                               = */
...
<%
	request.setCharacterEncoding ( "euc-kr" ) ; //기존 euc-kr 에서 한글이 깨져 변경함
    /* = -------------------------------------------------------------------------- = */
...
<%
    ...
    /* = -------------------------------------------------------------------------- = */
%>
	<h1>[KCP PG결제] <span> <%= g_ord_title %> </span></h1>
	<!-- 상단 문구 -->
	<div class="sample">
	        <p>이 페이지는 넥센타이어 렌탈 관련 결제를 요청하는 페이지입니다.<br />
	        <!--                             
	        현재 <span>테스트 모드</span>로 수행되고 있으므로 실제 결제는 발생하지 않습니다.
	        </p>
	         -->
...
%>
	  <tr>
	      <th>지불 방법</th>
	      <td><input type="text" name="pay_method_name" class="w100" value="<%= request.getParameter("pay_method_name") %>" maxlength="12" readonly /></td>
	      <td><input type="hidden" name="pay_method" value="<%= request.getParameter("pay_method_cd") %>"/></td>                        
	  </tr>
	  <!-- 주문번호(ordr_idxx) -->
	  <tr>
	      <th>주문 번호</th>
	      <td><input type="text" name="ordr_idxx" class="w200" value="<%= request.getParameter("ord_no") %>" maxlength="40" readonly /></td>
	  </tr>
	  <!-- 상품명(good_name) -->
	  <tr>
	      <th>상품명</th>
	      <td><input type="text" name="good_name" class="w100" value="<%= request.getParameter("good_name") %>" readonly /></td>
	  </tr>
	  <!-- 결제금액(good_mny) - ※ 필수 : 값 설정시 ,(콤마)를 제외한 숫자만 입력하여 주십시오. -->
	  <tr>
	      <th>결제 금액</th>
	      <td><input type="text" name="good_mny" class="w100" value="<%= request.getParameter("good_mny") %>" maxlength="9" readonly />원(숫자만 입력)</td>
	  </tr>
	  <!-- 주문자명(buyr_name) -->
	  <tr>
	      <th>주문자명</th>
	      <td><input type="text" name="buyr_name" class="w100" value="<%= request.getParameter("buyr_name") %>" readonly /></td>
	  </tr>
	  <!-- 주문자 E-mail(buyr_mail) -->
	  <tr>
	      <th>E-mail</th>
	      <td><input type="text" name="buyr_mail" class="w200" value="<%= request.getParameter("buyr_mail") %>" maxlength="30" readonly /></td>
	  </tr>
	  <!-- 주문자 연락처1(buyr_tel1) -->
	  <tr>
	      <th>전화번호</th>
	      <td><input type="text" name="buyr_tel1" class="w100" value="<%= request.getParameter("buyr_tel1") %>" readonly /></td>
	  </tr>
	  <!-- 휴대폰번호(buyr_tel2) -->
	  <tr>
	      <th>휴대폰번호</th>
	      <td><input type="text" name="buyr_tel2" class="w100" value="<%= request.getParameter("buyr_tel2") %>" readonly /></td>
	  </tr>
	  </table>
	  ...
	  <div class="btnset" id="display_pay_button" style="display:block">
	    ...
	    <a href="javascript:window.close();" class="home">취소</a>
	  </div>
...
%>    
    <input type="hidden" name="vcnt_expire_term" value="<%= request.getParameter("expire_term") %>"/> 
<%
    /*  가상계좌 입금 시간 설정하는 파라미터
         HHMMSS형식으로 입력하시기 바랍니다
         설정을 안하시는경우 기본적으로 23시59분59초가 세팅이 됩니다 */
%>         
         <input type="hidden" name="vcnt_expire_term_time" value="<%= request.getParameter("expire_time") %>"/> 
<%
...
%>    
        <input type="hidden" name="disp_tax_yn"     value="<%= request.getParameter("use_tax_yn") %>"/>
<%
