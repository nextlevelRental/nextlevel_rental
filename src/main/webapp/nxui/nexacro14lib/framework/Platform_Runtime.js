...
if (nexacro.Browser == "Runtime") {
	if (!nexacro._init_platform_runtime) {
		...
		nexacro._setHttpTimeout = function (v) {
			...
			application.httptimeout = application.proxytimeout = (timeout > 0) ? timeout : 30;
			nexacro.__setHttpTimeout(application.httptimeout);
		};

		nexacro._setHttpRetry = function (v) {
			var retry = nexacro._parseInt(v);
			application.httpretry = application.proxyretry = (retry > 0) ? retry : 3;
			nexacro.__setHttpRetry(application.httpretry);
		};

		nexacro._setHttpKeepalive = function (v) {
			var keepalive = nexacro._toBoolean(v);
			application.usehttpkeepalive = application.useproxykeepalive = keepalive;
			nexacro.__setHttpKeepalive(keepalive);
		};
