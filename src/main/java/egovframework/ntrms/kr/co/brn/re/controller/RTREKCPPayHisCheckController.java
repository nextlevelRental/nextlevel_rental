...
import egovframework.ntrms.kr.co.brn.re.service.RTREKCPPayHisCheckService;
...
@Controller
public class RTREKCPPayHisCheckController {
	
	protected Log logger = LogFactory.getLog(this.getClass());	

	@Resource(name="rTREKCPPayHisCheckService")
	RTREKCPPayHisCheckService rTREKCPPayHisCheckService;
	...
	@RequestMapping("/rtms/re/initRTREKCPPayHisCheck.do") 
	public ModelAndView initRTREKCPPayHisCheck(NexacroMapDTO xpDto, Model model) throws Exception {
		ModelAndView mav = new ModelAndView("nexacroMapView");
		try{
			Map <String, Object> inVar 			= xpDto.getInVariableMap();
			Map <String, DataSetMap> outDataset = xpDto.getOutDataSetMap();
	...
	@RequestMapping(value = "ntrms/re/cardApproveUploadFile.do")
	public void cardApproveUploadFile(NexacroMapDTO xpDto, HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		...
		Map result = null;
		Map resultUpdateRtre0043Check = null;
		...
		try{
		       		...
		       		if( (row != null) && (r != lastRow) ){
		       			for( short c = 0; c < 33; c++ ){								 //계좌승인 셀 갯수 -> 33개
		       				...
		       				if(cell != null){
		       					...
		       					if( c == 7 ){	paramMap.put("cardcomNm",format(cell));	}//카드종류
		       					if( c == 12 ){	paramMap.put("appAmt",format(cell));	}//거래금액
		       					if( c == 21 ){	paramMap.put("tno",format(cell));		}//거래번호
		       					if( c == 23 ){	paramMap.put("appDesc",format(cell));	}//거래구분
		       					//if( c == 24 ){	paramMap.put("cncAmt",format(cell));	}//취소금액
		       					if( c == 25 ){	paramMap.put("cncpAmt",format(cell));	}//취소가능금액
		       					if( c == 27 ){	paramMap.put("cncDay",format(cell));	}//취소일자
		       					if( c == 28 ){	paramMap.put("pcncDay",format(cell));	}//부분취소일자
		       				}
		       			}
			...
			result = rTREKCPPayHisCheckService.accApproveUploadFile(inDataset, inVar);
			...
			if( successCode == 0 ){
				...
				resultUpdateRtre0043Check = rTREKCPPayHisCheckService.resultUpdateRtre0043Check(inDataset, inVarUpdate);
				
				resVarList.add(NexacroConstant.ERROR_CODE, resultUpdateRtre0043Check.get("successCode"));
				resVarList.add(NexacroConstant.ERROR_MSG, resultUpdateRtre0043Check.get("returnMessage"));
			}else{
	...
	@RequestMapping(value = "ntrms/re/accApproveUploadFile.do")
	public void accApproveUploadFile(NexacroMapDTO xpDto, HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		DataSetMap tranInfo = xpDto.getTranInfoMap();
		Map <String, Object> inVar = xpDto.getInVariableMap();
		Map <String, Object> inVarUpdate = xpDto.getInVariableMap();
		Map <String, DataSetMap> inDataset = xpDto.getInDataSetMap();
		...
		try{
			...
			for( int i = 0 ; i < tmpNameList.size() ; i++){
				...
				HSSFWorkbook workbook = new HSSFWorkbook(fs);
		       	HSSFSheet sheet = workbook.getSheetAt(0);
		       	int rows = sheet.getPhysicalNumberOfRows();
		       	int lastRow = rows - 1;													 //마지막열은 총계열이므로 패스
		       	for( int r = 1 ; r < rows ; r++ ){										 //첫번째 행은 패스 -> r=1 부터시작
		       		HashMap<String, Object> paramMap = new HashMap<String, Object>();
		       		HSSFRow row = sheet.getRow(r);
		       		if( (row != null) && (r != lastRow) ){
		       			for( short c = 0; c < 22; c++ ){								 //계좌승인 셀 갯수 -> 22개
		       				HSSFCell cell = row.getCell(c);
		       				if(cell != null){
		       					if( c == 3 ){	paramMap.put("ordNo",format(cell));		}//주문번호
		       					if( c == 4 ){	paramMap.put("custNm",format(cell));	}//주문자
		       					if( c == 5 ){	paramMap.put("bankNm",format(cell));	}//은행명
		       					if( c == 7 ){	paramMap.put("appAmt",format(cell));	}//거래금액
		       					if( c == 11 ){	paramMap.put("stDesc",format(cell));	}//거래상태
		       					if( c == 12 ){	paramMap.put("appDay",format(cell));	}//승인일자
		       					if( c == 14 ){	paramMap.put("tno",format(cell));		}//거래번호
		       					if( c == 15 ){	paramMap.put("appDesc",format(cell));	}//거래구분
		       					if( c == 16 ){	paramMap.put("cncDay",format(cell));	}//취소일자
		       					//if( c == 17 ){	paramMap.put("cncAmt",format(cell));	}//부분취소금액
		       					if( c == 18 ){	paramMap.put("cncpAmt",format(cell));	}//취소가능금액
		       				}
		       			}
		       			paramMap.put(NexacroConstant.DATASET_ROW_TYPE,  DataSet.ROW_TYPE_INSERTED);
			       		paramMap.put("dvsn","I");
	...
	@RequestMapping("rtms/re/saveVerProcess.do") 
	public ModelAndView saveVerProcess(NexacroMapDTO xpDto, Model model, HttpServletRequest req) throws Exception {
		...
		try{
			...
			inVar.put("regId", loginInfo.get("userId"));
			Map result =  rTREKCPPayHisCheckService.saveVerProcess(inDataset, inVar);
	...
	@RequestMapping("/rtms/re/selectSRtre0043.do") 
	public ModelAndView selectSRtre0043(NexacroMapDTO xpDto, Model model) throws Exception {
		ModelAndView mav = new ModelAndView("nexacroMapView");
		try{
			Map <String, Object> inVar 			= xpDto.getInVariableMap();
			Map <String, DataSetMap> outDataset = xpDto.getOutDataSetMap();
			Map result = rTREKCPPayHisCheckService.selectSRtre0043(inVar, outDataset);
			List selectSRtre0043 = (List)result.get("selectSRtre0043");
			DataSetMap selectSRtre0043Map = new DataSetMap();
			selectSRtre0043Map.setRowMaps(selectSRtre0043);
			outDataset.put("selectSRtre0043Map", selectSRtre0043Map);
